...
1.  **Manual Re-indexing**:
    ...
    **Warning**: This *restarts the logic loop*. Ensure the crawl is truly done.
    ...
    ```bash
    # Only if you are sure WARCs are safe on disk!
    ha-backend db-shell
    # UPDATE archive_jobs SET status='indexed' WHERE id=6; -- Logic check: usually we want to re-run
    # Actually, to re-run indexing, the worker needs to pick it up.
    # Currently, worker only picks up 'queued'/'retryable'.
    # Manual indexing CLI is preferred:
    ```
